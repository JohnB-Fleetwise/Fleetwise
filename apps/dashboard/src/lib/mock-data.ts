...
const SEED_DELIVERIES = [
  ...
  { id: "DLV-004", driverId: "D-005", vehicleId: "V-005", status: "delivered", pLat: FTL.lat, pLng: FTL.lng, pStreet: "147 Broadway", pCity: "Fort Lauderdale", dSpread: 1.5, schedPickDays: -2, schedDropDays: -1, actualPickDays: -2, actualDropDays: -1, completedDays: -1, desc: "Medical supplies (refrigerated)", weight: 25, priority: "urgent", custName: "Bay Health", custPhone: "+1-954-555-1004", sigReq: true, payment: 0, dist: 1.8, dur: 10, createdDays: -3 },
...
export async function seedDatabase(): Promise<void> {
  ...
  for (const dlv of SEED_DELIVERIES) {
    ...
    await db.insert(deliveries).values({
      ...
      completed_at: dlv.completedDays != null ? new Date(iso(dlv.completedDays)) : null,
      updated_at: new Date(),
    });
  }
