...
//
// Routes under /dashboard and /_next are proxied to the FleetWise dashboard
...
// sudo so it can see (and the kill can) a process owned by another user;
...
// Proxy to the dashboard app on port 3001
async function proxyToDashboard(req: Request): Promise<Response> {
  const url = new URL(req.url);
  url.host = `localhost:${DASHBOARD_PORT}`;
  url.port = String(DASHBOARD_PORT);

  // Rewrite the request for the dashboard backend
  ...
  try {
    ...
    return new Response("Dashboard is not running", { status: 502 });
  }
}
  ...
  try {
    Bun.serve({
      ...
      async fetch(req) {
        ...
        // Serve static files from the marketing site
        if (pathname !== "/") {
          const file = Bun.file(CLIENT_DIR + pathname);
          if (await file.exists()) return new Response(file);
        }

        // SSR for marketing site pages
        return (
